...
표시한다. 대체된 `logsByIds`·`docsLogsByIds` delete method는 제거했다. 아직 검사하지 않은 다음
docs/admin-logs 경계를 추측하지 않는다.
...
| word-catalog 조회 | 완료 | 브라우저 검색·자동완성, 단어 상세 query, 고급 검색 Route Handler, 다운로드, 통계, 랜덤 연결 단어와 word combiner 후보 query 완료 |
...
1. 다음 docs/admin-logs 경계는 선택 삭제 다음에 남은 mutation의 실제 소비자와 요구사항부터 검사한다.
